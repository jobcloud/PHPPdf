...
abstract class XmlParser implements Parser
{
    ...
    /**
     * @param string|XMLReader $content
     * @return XMLReader
     * @throws Exceptions\InvalidTagException
     */
    private function getReader($content)
    {
        if ($content instanceof XMLReader) {
            ...
            if ($reader->name != static::ROOT_TAG) {
                throw new Exceptions\InvalidTagException(
                    sprintf('Root of xml document must be "%s", "%s" given.', static::ROOT_TAG, $reader->name)
                );
            }
    ...
    protected function read(XMLReader $reader)
    {
        ...
        if ($error) {
            ...
            throw new Exceptions\ParseException(
                sprintf(
                    'Xml parsing error "%s" in file "%s" on line %s on column %s',
                    $error->message,
                    $error->file,
                    $error->line,
                    $error->column
